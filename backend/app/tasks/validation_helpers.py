"""
Validation Helpers - Utilities for distributed validation processing.
...
def init_validation_stats(
    ...
    Args:
        ...
        dataset_id: Dataset (or Source) being validated
...
def read_csv_chunks(
    ...
    for chunk_df in pd.read_csv(
        ...
        if "ci_provider" in chunk_df.columns:
            ...
            if filtered_count > 0:
                logger.debug(
                    f"Filtered {filtered_count} rows with unsupported CI providers"
                )
...
def batch_create_source_builds(
    source_build_repo: SourceBuildRepository,
    builds: List[SourceBuild],
) -> int:
    """
    Batch insert SourceBuild records.

    Args:
        source_build_repo: Repository for SourceBuild
        builds: List of SourceBuild entities to insert
    ...
    result = source_build_repo.collection.insert_many(docs)
