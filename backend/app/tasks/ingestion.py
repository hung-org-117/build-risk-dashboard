...
@celery_app.task(bind=True, base=PipelineTask, name="app.tasks.ingestion.trigger_initial_scan")
def trigger_initial_scan(self: PipelineTask, repo_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not installation_id:
            raise ValueError("Repository missing installation_id. Cannot perform backfill without App installation.")
        ...
        logger.warning("Rate limit hit in trigger_initial_scan. Retrying in %s seconds.", wait)
        raise self.retry(exc=e, countdown=wait)
    except Exception as e:
        scan_repo.update_progress(job_id, status="failed", error=str(e))
        raise e

    return {"status": "completed", "job_id": job_id, "runs_found": len(runs) if 'runs' in locals() else 0}


@celery_app.task(bind=True, base=PipelineTask, name="app.tasks.ingestion.process_workflow_run")
def process_workflow_run(self: PipelineTask, repo_id: str, run: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    with get_app_github_client(self.db, installation_id) as gh:
        ...
        for job in jobs:
            ...
            try:
                ...
                if log_content:
                    logs_collected += 1
                    # TODO: Store log content or parse it
                    # For now, we just demonstrate we can download it
                    pass
            except Exception as e:
                logger.error(
                    "Failed to download logs for job %s in run %s (repo: %s): %s",
                    job_id, run_id, full_name, str(e), exc_info=True
                )
    except PipelineRateLimitError as e:
        wait = e.retry_after if e.retry_after else 60
        logger.warning("Rate limit hit in process_workflow_run. Retrying in %s seconds.", wait)
        raise self.retry(exc=e, countdown=wait)
