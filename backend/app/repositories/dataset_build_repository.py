...
class DatasetBuildRepository(BaseRepository[DatasetBuild]):
    ...
    def count_validated_builds(self, dataset_id: str) -> int:
        """Count validated builds for a dataset."""
        oid = self._to_object_id(dataset_id)
        if not oid:
            return 0
        return self.count({"dataset_id": oid, "status": "found"})
    ...
    def find_builds_with_run_ids(self, dataset_id: str) -> list[DatasetBuild]:
        """Find all validated builds with raw_run_id populated."""
        oid = self._to_object_id(dataset_id)
        if not oid:
            return []
        return self.find_many(
    ...
    def find_by_query(
        ...
        return self.find_many(
            query, sort=[(sort_by, sort_order)], skip=skip, limit=limit
        )
