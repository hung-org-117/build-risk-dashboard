...
class DashboardService:
    ...
    def _get_admin_extras(self) -> AdminDashboardExtras:
        """Get admin-only dashboard extras: pipeline stats and monitoring."""
        from datetime import datetime, timedelta

        # ============================================
        # 1. Training Scenario Pipeline Stats
        # ============================================
        scenario_collection = self.db["training_scenarios"]
        source_build_collection = self.db["source_builds"]

        total_scenarios = scenario_collection.count_documents({})
        queued_scenarios = scenario_collection.count_documents({"status": "queued"})
        processing_scenarios = scenario_collection.count_documents(
            {
                "status": {
                    "$in": [
                        "filtering",
        ...
        completed_scenarios = scenario_collection.count_documents({"status": "completed"})
        total_enriched_builds = source_build_collection.count_documents({})

        # ============================================
        # 2. Model Pipeline Stats (Repo Imports)
        # ============================================
        repo_config_collection = self.db["model_repo_configs"]

        total_repos = repo_config_collection.count_documents({})
        fetching_repos = repo_config_collection.count_documents({"status": "fetching"})
        ingesting_repos = repo_config_collection.count_documents({"status": "ingesting"})
        processing_repos = repo_config_collection.count_documents({"status": "processing"})
        processed_repos = repo_config_collection.count_documents({"status": "processed"})
        ...
        return AdminDashboardExtras(
            dataset_enrichment=DatasetEnrichmentStats(
                active_scenarios=total_scenarios,
                queued_scenarios=queued_scenarios,
                processing_scenarios=processing_scenarios,
            ...
            model_pipeline=ModelPipelineStats(
                ...
                ingesting_repos=ingesting_repos,
                processing_repos=processing_repos,
