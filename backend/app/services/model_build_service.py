...
class ModelBuildService:
            ...
            for training in training_builds:
                ...
                items.append(
                    BuildSummary(
                        ...
                        created_at=training.get("build_created_at") or raw.get("run_created_at"),
            ...
            for raw in raw_builds:
                ...
                items.append(
                    BuildSummary(
                        ...
                        prediction_status=(
                            training.get("prediction_status", "pending") if training else None
                        ),
    ...
    def get_recent_builds(self, limit: int = 10, current_user: dict = None) -> List[BuildSummary]:
        ...
        raw_cursor = self.db.raw_build_runs.find(query).sort("run_created_at", -1).limit(limit)
        raw_builds = list(raw_cursor)
        ...
        # Get repo names for raw_repo_ids
        raw_repo_cursor = self.db.raw_repositories.find(
            {"_id": {"$in": raw_repo_ids}}, {"full_name": 1}
        )
        repo_map = {doc["_id"]: doc["full_name"] for doc in raw_repo_cursor}

        items = []
        for raw in raw_builds:
            training = training_map.get(raw["_id"])
            repo_name = repo_map.get(raw.get("raw_repo_id"), "")
            items.append(
                BuildSummary(
                    _id=str(raw["_id"]),
                    repo_name=repo_name,
        ...
        for build_doc in unified_builds:
            ...
            unified_items.append(
                UnifiedBuildSummary(
                    ...
                    extraction_status=(
                        training_build.get("extraction_status") if training_build else None
                    ),
                    ...
                    prediction_status=(
                        training_build.get("prediction_status") if training_build else None
                    ),
                    predicted_label=(
                        training_build.get("predicted_label") if training_build else None
                    ),
                    prediction_confidence=(
                        training_build.get("prediction_confidence") if training_build else None
                    ),
                    prediction_uncertainty=(
                        training_build.get("prediction_uncertainty") if training_build else None
                    ),
                )
            )
