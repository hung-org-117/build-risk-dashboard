...
from app.tasks.model_ingestion import start_model_ingestion_pipeline
from app.tasks.model_processing import start_model_processing_pipeline
...
class RepositoryService:
    ...
    def bulk_import_repositories(
        ...
        for payload in payloads:
            ...
            # Prioritize App client (System) over User client to avoid 401s on expired user tokens
            # This works for:
            # 1. Org repos (App has access)
            # 2. Public external repos (App can read public data)
            # 3. Private external repos (Blocked later by is_private check)
            ...
            from app.tasks.pipeline.feature_dag.languages.registry import (
                LanguageRegistry,
            )
            ...
            if not main_lang or main_lang.lower() not in supported_langs:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=(
                        f"Repository '{payload.full_name}' language '{main_lang}' is not supported. "
            ...
            start_model_ingestion_pipeline.delay(
    ...
    def search_repositories(self, user_id: str, q: str | None) -> RepoSearchResponse:
        ...
        # prioritization: App Client > User Client
        if settings.GITHUB_INSTALLATION_ID:
            from app.services.github.github_client import get_app_github_client
    ...
    def trigger_sync(self, repo_id: str, user_id: str):
        ...
        from app.tasks.model_ingestion import orchestrate_model_ingestion
        ...
        if repo_doc.status in (
            ModelImportStatus.INGESTING,
            ModelImportStatus.PROCESSING,
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    def trigger_reprocess_failed(self, repo_id: str):
        ...
        """
        from app.tasks.model_processing import retry_processing_failures
        ...
        retry_processing_failures.delay(repo_id)
    ...
    def trigger_reingest_failed(self, repo_id: str):
        ...
        """
        from app.tasks.model_ingestion import reingest_failures
        ...
        reingest_failures.delay(repo_id)
    ...
    def get_import_progress(self, repo_id: str) -> dict:
        ...
        newest_processed_pipeline = [
            ...
            # Join with raw_build_runs to get ci_run_id
            {
                "$lookup": {
                    "from": "raw_build_runs",
        ...
        return {
            ...
            "checkpoint": {
                ...
                "last_processed_ci_run_id": last_processed_ci_run_id,
    ...
    def create_export_job(
        ...
        process_export_job.delay(job_id=str(job.id))
    ...
    def start_processing(self, repo_id: str) -> dict:
        ...
        start_model_processing_pipeline.delay(repo_config_id=repo_id)
