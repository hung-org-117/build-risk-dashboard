...
df["tr_status_num"] = (
    df["tr_status"].map({"passed": 0, "failed": 1}).fillna(-1).astype(int)
)
...
model = BayesianRiskModel(
    temporal_dim=len(TEMPORAL_FEATURES), static_dim=len(STATIC_FEATURES)
)
...
mean_prob, uncertainty = mc_dropout_predict(model, seq, static, lengths, n_samples=30)
